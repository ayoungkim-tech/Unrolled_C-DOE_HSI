function [net,stats] = cnn_train_dag_v3(net, imdb, getBatch, varargin)
...
for t=1:params.batchSize:numel(subset)
  ...
  label_id = net.getVarIndex('label');
  label = net.vars(label_id).value;
  label_cpu = gather(label);
  fprintf(['  PSNR: ' num2str(psnr(single(label_cpu),output_cpu)) '\n']) ;
    
  figure(2);
  warning('off','images:imshow:magnificationMustBeFitForDockedFigure');
  h = net.params(1).value;
  
  if size(h,1) == 1 % Zernike coefficients
    h = Zern_to_phi0(net.layers(1).block.opts(1).Zern,h);
  end
    
  subplot(221);
  imagesc(h); colorbar;
  title(net.params(1).name);
  %hder = net.params(1).der;
  %subplot(222);
  %imagesc(hder); colorbar;
  %title([net.params(1).name,'-der']);
  subplot(223);
  imshow(output(:,:,12,1));
  title('Output');
  subplot(224);
